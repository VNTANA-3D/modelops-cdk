...
function renderWorkerScript(vars: {
  region: string;
  accountId: string;
  image: string;
  tag: string;
...
export class ModelopsDeadlineStack extends cdk.Stack {
  ...
  private getFleetRole(
    ...
    const ecrRepoArn = this.buildEcrRepoArn();
  ...
  /**
   * Builds the ECR repository ARN from the image URI in config.
   *
   * Image format: <account>.dkr.ecr.<region>.amazonaws.com/<repo-path>
   */
  private buildEcrRepoArn(): string {
    const match = this.#config.image.match(
      /^(\d+)\.dkr\.ecr\.([^.]+)\.amazonaws\.com\/(.+)$/,
    );

    if (!match) {
      return `arn:aws:ecr:${this.#config.region}:${this.#config.account!}:repository/*`;
    }
